...
> **管线定位**：本文档是 Phase 1 `frontend-design` skill 的**输入参考**。`frontend-design` 产出具体 UI 方案后，由 `writing-plans` 转化为实施步骤。设计令牌（色板、圆角、阴影、字体）是组件实现的**权威标准**。
...
- **入口**：侧边栏左下角齿轮按钮
- **布局**：760×520 模态框，左 176px 导航栏 + 右自适应内容区
- **导航项**：外观（主题切换）、API 配置
- **外观页**：深色/浅色主题卡片，带预览色块，选中态强调色边框+发光
- **API 配置页**：配置列表 + 添加/编辑表单 + 获取模型按钮 + 测试连接按钮
- **关闭方式**：Esc、点击遮罩、点击关闭按钮
